...
public class FlightServiceImpl implements FlightService {
	...
	public List<Flight> findFlights(SearchQuery query) {
		...
		flights = flights.stream().filter(flight -> flight.getInventory().getAvailableSeats() >= query.getTravellers())
	...
	public List<Flight> listScheduledFlights(@PathVariable String origin, @PathVariable String destination,
		...
		return flightDao.findByOriginAndDestinationAndFlightDate(origin, destination, flightDate).stream()
				.filter(flight -> flight.getInventory().getAvailableSeats() >= travellers).collect(Collectors.toList());
	}
